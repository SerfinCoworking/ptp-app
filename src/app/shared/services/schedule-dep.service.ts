...
import { ICalendarList, IPeriod } from '@shared/models/schedule';
...
export class ScheduleDepService {
  ...
  getSchedules(search?: string, sort?: string, schedulePage?: number, periodPage?: number, objectiveId?: string): Observable<ICalendarList> {
    ...
    return this.http.get<ICalendarList>(`${environment.API_END_POINT}/schedules-dep`, {params: params});
  }
  ...
  getSchedulePeriods(scheduleId: string, periodPage?: number, stream: boolean = false): Observable<ICalendarList> {
    ...
    return this.http.get<ICalendarList>(`${environment.API_END_POINT}/schedule-by-id-dep/${scheduleId}`, {params: params, headers: {'Silent': (stream ? 'yes' : 'no') } });
  }

  // NEW RECORD
  newRecord(): Observable<IObjective[]> {
    return this.http.get<IObjective[]>(`${environment.API_END_POINT}/schedules-dep/new`);
  }

  // CREATE EMPTY SCHEDULE
  create(objective: string): Observable<any>{
    return this.http.post<any>(`${environment.API_END_POINT}/schedules-dep`, { objective });
  }
  
  // GET SCHEDULE
  getSchedule(id: string): Observable<any>{
    return this.http.get<any>(`${environment.API_END_POINT}/schedules-dep/${id}`);
  }
